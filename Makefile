...
build:
	...
	export os_arch_seen="" && echo '$(BUILD_PLATFORMS)' | tr ';' '\n' | while read -r os arch buildx_platform suffix base_image addon_image; do \
		...
		if ! (set -x; cd ./$(CMDS_DIR)/$(CMDS) && CGO_ENABLED=0 GOOS="$$os" GOARCH="$$arch" go build $(GOFLAGS_VENDOR) -a -ldflags '$(FULL_LDFLAGS)' -o "$(abspath ./bin)/$(CMDS)$$suffix" .); then \
			echo "Building $(CMDS) for GOOS=$$os GOARCH=$$arch failed, see error(s) above."; \
			exit 1; \
		fi; \
...
container: build
	docker build -t $(CMDS):latest -f $(shell if [ -e ./$(CMDS_DIR)/$(CMDS)/Dockerfile ]; then echo ./$(CMDS_DIR)/$(CMDS)/Dockerfile; else echo Dockerfile; fi) --label revision=$(REV) .

push: container
	set -ex; \
	push_image () { \
		docker tag $(CMDS):latest $(IMAGE_NAME):$$tag; \
...
push-multiarch: check-pull-base-ref build
	...
	dockerfile_linux=$$(if [ -e ./$(CMDS_DIR)/$(CMDS)/Dockerfile ]; then echo ./$(CMDS_DIR)/$(CMDS)/Dockerfile; else echo Dockerfile; fi); \
	dockerfile_windows=$$(if [ -e ./$(CMDS_DIR)/$(CMDS)/Dockerfile.Windows ]; then echo ./$(CMDS_DIR)/$(CMDS)/Dockerfile.Windows; else echo Dockerfile.Windows; fi); \
	...
	pushMultiArch () { \
		...
		echo "$$build_platforms" | tr ';' '\n' | while read -r os arch buildx_platform suffix base_image addon_image; do \
			...
			docker buildx build --push \
				...
				--build-arg binary=./bin/$(CMDS)$$suffix \
				--build-arg ARCH=$$arch \
